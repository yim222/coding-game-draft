 ...
 *         Izhar Mashkif
 * 
 *         Hi, I am A software development consulting contractor. * Working
 *         world wide, located in Israel. Specialization in SaaS systems, web,
 *         Mobile application, devices programming
 * 
 * 
 * 
 *         Handling all software development process - Planning, implementing,
 *         Testing, Deployment. Smart and efficient project management.
 * 
 *         Do things simpler and better.
 * 
 * 
 *         https://www.linkedin.com/in/izhar-mashkif-86a01913a/
 * 
...
//		String [] piecesProvided = {"g5", "d2"};
		...
		// CASE 4 --> PASSED
		String[] inputs = { "d5" };
		int[] colors = { 0, 0, 1 };// like the test
		String[] piecesProvided = { "g5", "d2", "d7" };
...
//            String onePiece = in.next();
		...
		// creating the rook..
		Rook rook = new Rook(new Position(rookPosition));
...
class Rook extends Piece {
	// Four available routes for rook
	...
	Comparator<Move> comprator = new CompareByMoves();
	SortedSet<Move> movesList = new TreeSet<Move>(comprator);

	public Rook(Position position) {
		super(position, 'R', 0);
		// Step 1 - getting the relevant blockers to this rook
		getAndFilterBlockerPieces();
		//
		setAvailableRoutes();
		generateMovesList();
	...
	public void getAndFilterBlockerPieces() {

		blockerPieces = new ArrayList<>(Board.pieces);

		blockerPieces = blockerPieces.stream().filter(piece -> piece.getPosition().getColumn() == position.getColumn()
				|| piece.getPosition().getRow() == position.getRow()).collect(Collectors.toList());
		...
		blockerPieces.stream().forEach((piece) -> {
			...
			if (piecePosition.getColumn() < position.getColumn()) {// left blocker
				if (blockerPiecesMap.get(Direction.LEFT) == null
						|| blockerPiecesMap.get(Direction.LEFT).getPosition().getColumn() < piecePosition.getColumn()) {// if
																														// it's
																														// closer
																														// to
																														// the
				...
				if (blockerPiecesMap.get(Direction.RIGHT) == null || blockerPiecesMap.get(Direction.RIGHT).getPosition()
						.getColumn() > piecePosition.getColumn()) {// same concept
	...
	// sorted set - https://www.geeksforgeeks.org/sortedset-java-examples/
	public void generateMovesList() {
		...
		for (Map.Entry<Direction, Route> entry : availableRoutes.entrySet()) {
			...
			boolean horizontalRoute = entry.getKey().equals(Direction.LEFT) || entry.getKey().equals(Direction.RIGHT);
			int indexToCheckOpp = entry.getKey().equals(Direction.LEFT) || entry.getKey().equals(Direction.DOWN)
					? entry.getValue().from
					: entry.getValue().to;
			for (int i = entry.getValue().from; i <= entry.getValue().to; i++) {
				Move move;
				if (horizontalRoute) {
					// case left\right - we consider the range of the route as columns char (+96)
					// with the line of this rook.
					// for example if it's horizontal route -
					...
					// case up\down - we consider the col of this rook as the columns, and add to
					// them the range of the
					// route as row number:
...
/**
 * Generates correct route per provided data.
 * 
...
class Route {// to use position
	...
	// In real world application we suppose to do per each piece a proper route,
	// handle other directions too, and make better polymorphism
	...
	@Override
	public String toString() {
		return "Route [ position=" + position + ", direction=" + direction + ", describe=" + describe + ", blockerType="
				+ blockerType + ", blockerPiece=" + blockerPiece + ", connectionSign=" + connectionSign + ", from="
				+ from + ", to=" + to + "]";
	}
	...
	public void generatePathsForRook() {// to use position
		...
		case LEFT:// should take the rook line on first col to the before the rookCol. For example
			...
			if (currentPiece.getPosition().getColumn() - 96 == 1) {// It's next to the wall
				...
				if (blockerType == Blocker.WALL) {
					...
					from = blockerPiece.getPosition().getColumn() - 96;

				}

				to = currentPiece.getPosition().getColumn() - 96 - 1;
		...
		case RIGHT:// should take the rook line from the next column of the rook to the the last.
					// For example
		...
		case DOWN:// should take the rook columns from the first line to the before the
					// position.getRow(). For example
		...
		case UP:// should take the rook columns from the next line of the rook to the the last
				// line. For example
...
class CompareByMoves implements Comparator<Move> {
 ...
 * @author lingar
 *Example: 
 *         piece = Rd5 to = d3 isCapture = true;
 *
 *         result: combination =  Rd5xd3
 *
 *         piece = Rd5 to = a3 isCapture = false;
 *
 *         result: combination = Rd5-a3
 */
class Move {
	private Piece piece;
	...
	public Move(Piece piece, Position toPosition, boolean isCapture) {
		...
		combination = piece.getToolSign() + "" + piece.getPosition().getPositionString() + (isCapture ? 'x' : '-')
				+ toPosition.getPositionString();
	}
